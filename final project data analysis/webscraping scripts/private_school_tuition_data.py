...
def get_college_tuition_data(college):
	...
	try:
		# Get Tuition Change
		row = table.find(string="Tuition and fees").parent.parent
		cols = row.find_all("td")
		college['Tuition and fees'] = cols[5].get_text()
...
def is_invalid_college(college):
	print(college['Tuition and fees'] == "-")
	print(college['name'])
	return college['Tuition and fees'] == "-"
...
colleges = [college for college in colleges if not is_invalid_college(college)]

for college in colleges:
	print(repr(str(college['name'])) + ", " + college['Tuition and fees'])
