...
def get_college_net_price_data(college):
	"""Retrieves college net price data and adds to college
	:param college: college dictionary container
	"""
	response = requests.get(ROOT_URL + college['url'])
	soup = BeautifulSoup(response.text, "html.parser")

	expenses = soup.find("div", attrs={"id": "netprc"})
	table = expenses.find("tbody")

	try:
		
		# Get In-state Net price 
		row = table.find(string="Average net price").parent.parent
		cols = row.find_all("td")
		college['In-state'] = cols[2].get_text()

		
	except (AttributeError, TypeError):
		print('\033[93m' + college['name'] + " has no net price data, skipping!!" + '\033[0m')
		college['In-state'] = "-"
...
# Get additional net price data for each college
for college in colleges:
	print(college['name'] + ": Retrieving net price data")
	get_college_net_price_data(college)

for college in colleges:
	print(repr(str(college['name'])) + ": In-state = " + college['In-state'])
